...
static int
mt9v111_write( const uint8_t reg, const uint16_t val )
{
    uint8_t tmp[3] = { 0 };
...
static uint16_t
mt9v111_read( const uint8_t reg )
{
    uint8_t buf[2] = { 0 };
    buf[0] = reg;
...
mt9v111_write_registers( const Register_t * registers )
{
    ...
    while( registers[i].reg != 0 )
    {
        ...
        if( registers[i].mask )
        {
            ...
            value = mt9v111_read( registers[i].reg );
        ...
        mt9v111_write( registers[i].reg, registers[i].value );
...
{
    printk("%s sensor %p\r\n", __func__, sensor);
    mt9v111_write( reg_addr, reg_data );
    return 0;
}
